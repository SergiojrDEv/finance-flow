...
function countExpenseCategories(transactions) {
  return new Set(
    transactions
...
function buildHealth({ income, expenses, investments, available, hasTransactions }) {
  if (!hasTransactions) {
    return {
      score: 0,
      status: "empty",
...
export function buildFinancialSummary(transactions = []) {
