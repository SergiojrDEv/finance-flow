...
import { createSupabaseCatalogRepository } from "../infrastructure/repositories/supabaseCatalogRepository.js";
...
export function createSupabaseModule(deps) {
  ...
  const v2SupabaseRuntime = createV2SupabaseRuntimeFacade({
    state,
    deps,
    v2SyncFacade,
    supabaseShadow,
    ...
    handleCloudError,
    renderCloudStatus,
    createCatalogRepository: () => createSupabaseCatalogRepository(state.supabaseClient),
  ...
  async function syncToSupabase() {
    ...
    if (supportsV2) {
      try {
        ...
        const refs = await v2SupabaseRuntime.syncSettings(userId);
        await v2SupabaseRuntime.syncTransactions(userId, refs);
      } catch (error) {
        handleCloudError(error);
        return;
      }
  ...
  async function pullFromSupabase(options = {}) {
    ...
    if (supportsV2) {
      await v2SupabaseRuntime.pull({ options });
      return;
    }
